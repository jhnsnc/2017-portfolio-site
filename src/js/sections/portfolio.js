...
export function setupPortfolioSection() {
  const portfolioSection = document.getElementById('portfolio');
  if (!portfolioSection) { return; }

  // cache DOM elements
  const textContainer = portfolioSection.querySelector('.portfolio-section__inner-content');
  const textHeadings = [...textContainer.querySelectorAll('.section-heading')];
  const listContainer = portfolioSection.querySelector('.portfolio-list__container');
  const detailsContainer = document.querySelector('.portfolio-details');

  setupTextAdjustment(textContainer, textHeadings);
  setupPortfolioEntries(listContainer, detailsContainer);
}

function setupTextAdjustment(textContainer, textHeadings) {
...
function setupPortfolioEntries(listContainer, detailsContainer) {
  ...
  portfolioData.forEach(entryData => {
    ...
    newEntry.listItem.addEventListener('click', () => { showPortfolioDetails(newEntry.title); });
    newEntry.detailView.querySelector('.portfolio-details__close-button').addEventListener('click',
